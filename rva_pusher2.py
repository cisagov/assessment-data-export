...
def grab_csv_from_jira(filter):
    ...
    subprocess.call(['curl -k {}:{} https://jira.ncats.cyber.dhs.gov/sr/jira.issueviews:searchrequest-csv-current-fields/{}/SearchRequest-{}.csv\?delimiter\=, -o {}'.format(username,password,filter,filter,JIRA_FILE)], shell=True)
    ...
    for row in csv.reader(assessments):
        ...
        try:    
            db.rva.insert_one({
                ...
                "summary": row[0],
                'asmt_name': row[22],
                'requested_services': [row[23],row[24],row[25],row[26],row[27],row[28],row[29],row[30]],
                ...
                'operators': [row[44],row[45],row[46],row[47],row[48]],
